...
export function openCoordsGui()
{
    ...
    const CoordsGui = new JavaAdapter(WindowScreen, {
        init() {
            coordsWindow(1, 1, "Spiral", "cw spiral toggle", "https://i.ibb.co/mrzmh6ht/image.png", true, "Do /cw spiral to see image. (image isn't loading.)").setChildOf(this.getWindow())
            coordsWindow(1, 2, "Throne", "cw throne toggle", "https://i.ibb.co/wZHNYv3B/image.png", true, "Go back of throne. (image isn't loading)").setChildOf(this.getWindow())
            coordsWindow(1, 3, "Yog", "cw yog toggle", "https://i.ibb.co/5XDgpnZq/image.png", true, "Go to the leftmost corner of the topaz crystal facing bal close to bal. (image isn't loading)").setChildOf(this.getWindow())
            coordsWindow(2, 1, "Divans", "cw divans toggle", "https://i.ibb.co/sp1cqN2J/image.png", true, "Go to the middle of jade crystal. (image isn't loading)").setChildOf(this.getWindow())
            coordsWindow(2, 2, "Automatons", "cw automatons toggle", "https://i.ibb.co/BKn1p53r/QasAWf7.png", true, "Sit in pot somewhere (check image in command) (image isn't loading)").setChildOf(this.getWindow())
